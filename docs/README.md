...
1. 역 관리
- 지하철 역 등록하기 & 삭제하기
- 등록된 모든 지하철 역의 목록 출력하기
[예외처리]
- 이미 저장되어 있는 역 이름 입력시
- 역 이름 2글자보다 적게 입력시

2. 노선 관리
- 지하철 노선 등록하기(상행 종점과 하행 종점 입력) & 삭제하기
- 등록된 모든 지하철 노선 목록 출력하기
[예외처리]
- 이미 저장되어 있는 노선 이름 입력시
- 지하철 노선 이름 2글자보다 적게 입력시

3. 구간 관리
- 지하철 노선에 지하철 역(구간)을 원하는 위치에 추가하기
- 노선에 등록된 역(구간) 삭제하기
[예외처리]
- 노선 목록에 등록되지 않은 노선을 등록하려고 할 시
- 역 목록에 등록되지 않은 역을 등록하려고 할 시
- 이미 해당 노선에 등록되어 있는 역을 다시 등록하려고 할 시

4. 지하철 노선도 출력
- 저장된 노선의 모든 역을 순서대로 출력한다.

5. 대쉬보드
- 해당 시점의 선택 옵션 표시하기
- 옵션을 선택하면 해당 기능 스택에 등록하기
- 돌아가기나 종료를 선택하면 스택 비우기
메인 대쉬보드를 인터페이스로 구현하고 입력 조건에 맞추어 해당하는 대쉬보드로 이동할 것
[예외처리]
- 존재하지 않는 옵션을 선택할 시

### 비지니스 로직과 UI 로직을 분리하여야 하지만 아직 적절한 로직이 떠오르지 않아 먼저 이 방식으로 구현할 것. 추후 필히 리팩토링 진행
