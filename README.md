...
- ####  입력
    ...
    - 등록할 역의 이름을 입력 받는다.
    - 삭제할 역의 이름을 입력 받는다.
    - 등록할 노선의 이름, 상행 종점과 하행 종점을 입력 받는다.
    - 삭제할 노선의 이름을 입력 받는다.
    - 삭제할 구간의 노선 이름과 해당 노선에 포함된 제거할 역 이름을 입력 받는다.
    - 추가할 구간의 노선 이름과 해당 노선에 추가할 역, 순서를 입력받는다.

- #### 예외 처리
    - 메뉴에 없는 기능을 입력할 수 없다.
    - 노선과 역 이름은 2글자 이상이어야 한다.
    - 노선에 등록된 역은 삭제할 수 없다.
    - 중복된 역은 등록할 수 없다.
    - 중복된 노선은 등록할 수 없다.
    - 노선 등록 시 입력 받는 두 종점역은 같을 수 없다.
    - 구간 등록 시 노선에 이미 포함된 역을 중복 등록 할 수 없다.
    - 구간 등록 시 순서는 1이상이고, 노선의 기존 사이즈보다 큰 숫자는 입력할 수 없다.
    - 구간 삭제 시 노선에 포함된 역이 두개 이하일 때는 역을 삭제할 수 없다.
