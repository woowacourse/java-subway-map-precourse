...
등록  
 - 중복된 역 이름을 등록 했을때
 - 2글자 미만인 역 이름을 등록 했을때

삭제  
 - 노선에 등록된 역을 삭제하려고 했을 때
 - 존재하지 않는 역을 삭제하려고 했을 때
...
등록
 - 중복된 노선 이름을 등록 했을때
 - 2글자 미만인 노선 이름을 등록했을때

삭제
  - 존재하지 않는 노선을 삭제하려고 했을때
...
- 역과 역사이에 새로운 역이 추가될 수 있다.
- 노선에서 갈래길은 생길 수 없다.

**예외처리**  
 - 없는 노선에 구간을 추가하려고 했을때
 - 없는 역을 구간으로 추가하려고 했을때
 - 노선에 이미 존재하는 역을 구간으로 추가하려고 했을때
 - 입력한 순서가 유효한 구간 순서가 아닐때
 - 구간 추가를 통해 갈래길이 생기는 상황이 발생할때 (구현중)

참고)  
역과 역사이를 구간이라고 하고 이 구간들의 모음이 노선
...
**예외 처리**  
  - 존재하지 않는 노선을 입력했을때
  - 존재하지 않는 역을 삭제하려고 했을때
  - 노선에 포함된 역이 두개 이하일때, 역을 제거하려 했을때
