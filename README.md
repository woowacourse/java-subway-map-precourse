...
<br>
## 구조 계획
- 각 메뉴 운영을 담당하는 Controller
- 저장된 데이터를 불러오는 Repository
- 비즈니스 로직을 담당하는 Service
  - 예외 처리를 담당하는 Validation
- 여러 클래스에 중복으로 사용하는 기능을 모아놓은 Utils
  - 입출력을 담당하는 InputUtils
  - 화면 출력을 담당하는 PrintUtils
  - 더미 데이터 저장을 위한 DummyUtils
...
### 지하철 노선 관련 기능
- [x] 지하철 노선을 등록하고 삭제할 수 있다. (노선 등록 시 상행 종점역과 하행 종점역을 입력받는다.)
...
### 지하철 구간 삭제 기능
- [x] 노선에 등록된 역을 제거할 수 있다(종점을 제거할 경우 다음 역이 종점이 된다.)
