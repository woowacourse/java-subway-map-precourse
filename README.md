...
- **지하철 역**
- [ ] 지하철 역을 등록한다.
    - [ ] 예외 1. 지하철 역 이름이 2글자 이하면 예외를 발생시킨다.
    - [ ] 예외 2. 이미 등록된 이름은 예외를 발생시킨다.
- [ ] 지하철 역을 삭제한다.
    - [ ] 예외 1. 해당 이름을 가진 역이 없으면 예외를 발생시킨다.
    - [ ] 예외 2. 노선에 등록된 역은 삭제할 수 없고 예외를 발생시킨다.
- [ ] 지하철 역 목록을 조회한다.
- [ ] 지하철 역 이름을 키로 지하철 역 객체를 찾을 수 있다.
- **지하철 노선**
- [ ] 지하철 노선을 등록한다.
    - [ ] 노선 이름을 입력 받는다.
        - [ ] 예외 1. 노선 이름이 2글자 이하면 예외를 발생시킨다.
        - [ ] 예외 2. 이미 등록된 이름은 예외를 발생시킨다.
    - [ ] 상행 종점역과 하행 종점역을 입력받는다.
- [ ] 지하철 노선을 삭제한다.
    - [ ] 예외 1. 해당 이름을 가진 노선이 없으면 예외를 발생시킨다.
- [ ] 지하철 노선에 구간을 추가한다. (역을 추가한다.)
    - [ ] 구간을 추가할 노선을 입력받는다.
        - [ ] 예외 1. 해당 이름을 가진 노선이 없으면 예외를 발생시킨다.
    - [ ] 노선에 추가할 역 이름을 입력한다.
        - [ ] 예외 1. 해당 이름을 가진 역이 없으면 예외를 발생시킨다.
    ...
    - [ ] 역이 추가된 순서가 0이나 마지막 순서라면 해당 역을 노선의 종점으로 변경한다. 
- [ ] 지하철 노선에 구간을 삭제한다. (역을 삭제한다.)
    - [ ] 구간을 삭제할 노선을 입력받는다.
        - [ ] 예외 1. 해당 이름을 가진 노선이 없으면 예외를 발생시킨다.
    - [ ] 노선에 삭제할 역 이름을 입력한다.
        - [ ] 예외 1. 해당 이름을 가진 역이 없으면 예외를 발생시킨다.
        - [ ] 예외 2. 노선의 사이즈가 2 이하라면 예외를 발생시킨다.  
  - [ ] 삭제된 역이 종점이라면 그 다음 역을 노선의 종점으로 변경한다.
### 입출력 기능
- [ ] 문자열을 입력받을 수 있다.
- [ ] 정수값을 입력받을 수 있다.
    - [ ] 예외 1. 정수가 아니면 예외를 발생시킨다.
- [ ] 결과물을 화면에 출력한다.     
### 메뉴 기능
- [ ] 전체 메뉴를 출력하고 입력받으면 해당 도메인의 세뷰메뉴로 연결한다.
    - [ ] 예외 1. 정해진 메뉴 외에 다른 값을 입력받으면 예외를 발생시킨다. 
- [ ] 해당 도메인의 세부 메뉴를 출력하고 입력받으면 해당 도메인으로 연결한다.
    - [ ] 예외 1. 정해진 메뉴 외에 다른 값을 입력받으면 예외를 발생시킨다.     
- [ ] 종료에 해당하는 커맨드를 입력받으면 프로그램을 종료시킨다.
