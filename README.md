...
- 지하철 노선 생성 기능
  ...
  - 노선 등록 시 상행 종점역과 하행 종점역을 입력받는다.
    - 상행 종점역과 하행 종점역은 존재 해야 한다.
    - 상행 종점역과 하행 종점역은 같은 역일 수 없다.

- 지하철 노선 등록 기능

  - 중복된 지하철 노선 이름은 등록 될 수 없다.
...
- 지하철 구간 추가 기능 (지하철 노선에 역을 추가하는 기능)

  - 이미 노선에 포함되어 있는 역은 구간 으로 추가 될 수 없다.
  - 지하철 역으로 등록된 역이 구간으로 추가 되어야 한다.
  - 입력된 순서는 1(상행 종점) 이상 등록된 구간의 사이즈 + 1 (하행 종점 뒤에 등록하기 위해) 이하 여야 한다.

- 지하철 구간 삭제 기능

  - 입력한 역이 노선에 존재해야 한다.
  - 종점을 제거할 경우 다음 역이 종점이 된다.
    - 상행 종점이 제거 된 경우 앞의 역이 상행 종점이 된다.
    - 하행 종점이 제거 된 경우 이전의 역이 하행 종점이 된다.
  - 노선에 포함된 역이 두개 이하일 때는 역을 제거할 수 없다.

- 지하철 노선에 등록된 역 조회 기능

  - 노선의 상행 종점 부터 하행 종점 까지 연결된 순서대로 역 목록 조회가능
