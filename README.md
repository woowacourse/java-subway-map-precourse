...
## ✅ 구현할 기능 목록
- [ ] 초기 설정 기능
  - [기능 요구사항](#-기능-요구사항)에 맞춰 지하철 역, 노선, 구간을 셋팅
- [x] 지하철 노선도 관리 인터페이스 
  - 메뉴에 존재하지 않는 입력이 있을시 에러 메시지를 출력
- [x] 지하철 역 등록 기능
  - 역 이름이 2글자 이상인지 확인
  - 중복되는 역 이름이 있는지 확인
- [x] 지하철 역 삭제 기능
...
- [x] 지하철 노선 등록 기능
  - 노선 이름이 2글자 이상인지 확인
  - 중복되는 노선 이름이 있는지 확인
  - 입력받은 상행 종점역과 하행 종점역이 2글자 이상인지 확인
  - 입력받은 상행 종점역과 하행 종점역이 등록되어 있는지 확인
  - 입력받은 상행 종점역과 하행 종점역이 같은지 확인
- [x] 지하철 노선 삭제 기능
  - 노선 이름이 2글자 이상인지 확인
  - 존재하는 이름의 노선인지 확인
- [x] 지하철 노선 목록 출력 기능
  - 지하철 노선이 없을 경우 에러 메시지 출력
- [ ] 지하철 구간 추가 기능
  - 입력받은 역이 등록되어 있는지 확인
  - 입력받은 노선이 등록되어 있는지 확인
  - 이미 해당 노선에 역이 등록되어 있는지 확인
- [ ] 지하철 구간 삭제 기능
  - 입력받은 역이 등록되어 있는지 확인
  - 입력받은 노선이 등록되어 있는지 확인
  - 입력받은 역이 노선에 등록되어 있는지 확인
  - 노선에 역이 2개 이상인지 확인
- [ ] 지하철 노선에 등록된 역 조회 기능
  - 등록된 노선이 없는 경우 에러 메시지 출력
  
<br>

## 🛠설계
### view(package)
지하철 관리 프로그램에 나올 수 있는 화면들을 관리한다. 해당 화면에 있는 기능 목록과 기능에 해당하는 메소드를 관리한다.
- View(class): 모든 뷰들의 부모 클래스
  - MainView(class): 메인 화면
  - StationManagementView(class): 지하철 역 관리 화면
...
### Scene(class)
지하철 관리 프로그램에서 사용되는 하나의 화면들의 그룹
### CommandFunction(Interface)
method inference를 위한 인터페이스
