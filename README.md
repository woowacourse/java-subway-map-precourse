...
- (Station) 역 삭제 
    - 노선 등록 여부 확인
    - `[ERROR]` 등록되어 있지 않은 역인 경우 
- (Station) 역 목록 조회
- (Line) 노선 등록 
    - 중복체크
    - 2자 이상 사이즈 체크
    - 상행, 하행 입력
    - `[ERROR]` 구간에 넣으려는 역의 위치가 숫자가 아닐 경우
    - `[ERROR]` 구간에 넣으려는 역의 위치가 노선이 가진 역의 수 + 1 보다 큰 경우
- (Line) 노선 삭제
    - `[ERROR]` 등록되어 있지 않은 노선인 경우 
- (Line) 노선 목록 조회
- (Section) 노선에 구간 추가
    - 하나의 역은 여러 노선에 추가 가능
    - 역 사이, 맨 앞, 맨 뒤 추가 가능
    - 갈래길 불가
    - `[ERROR]` 노선에 등록되어 있는 역인 경우
    - `[ERROR]` 등록되어 있지 않은 역인 경우 
    - `[ERROR]` 등록되어 있지 않은 노선인 경우 
- (Section) 노선에 등록된 역 제거
    - 종점일 경우 다음역이 종점
    - `[ERROR]` 해당 노선에 포함된 역이 두개일 경우 제거 불가
    - `[ERROR]` 노선에 등록되지 않은 역을 입력한 경우
- (Section) 노선에 등록된 역 조회
- 출력 결과 `[INFO]` 를 붙여서 출력
- 에러 발생 시 `[ERROR]` 를 붙여서 출력
- 예외 상황 추가
    - `[ERROR]` 기능 선택 범위 외의 선택을 하는 경우
