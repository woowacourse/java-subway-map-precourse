...
public class SectionService extends BaseService {
    ...
    public static void register() {
        OutputView.println(REGISTER_SECTION_LINE_NAME_QUESTION);
        Line line = LineRepository.getLine(InputView.getLineName());
        OutputView.println(REGISTER_SECTION_STATION_NAME_QUESTION);
        Station station = StationRepository.getStation(InputView.getStationName());
        OutputView.println(REGISTER_SECTION_ORDER_NUMBER_QUESTION);
        int order = InputView.getOrder();
        line.add(order - 1, station);
        OutputView.println(REGISTER_SECTION_SUCCESS);
    }

    public static void delete() {
        OutputView.println(DELETE_SECTION_LINE_NAME_QUESTION);
        Line line = LineRepository.getLine(InputView.getLineName());
        OutputView.println(DELETE_SECTION_STATION_NAME_QUESTION);
        Station station = StationRepository.getStation(InputView.getStationName());
